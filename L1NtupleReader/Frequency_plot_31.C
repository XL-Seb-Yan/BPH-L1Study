...
void Frequency_plot_31(const TString samplename="ZeroBias2018",
                      const int nEvents = -1,
  ...
  std::string line;
    ...
    for(UInt_t ientry=0; ientry<Evt2Process; ientry++){
      ...
      for(int EgEtThr = 0; EgEtThr < 10; EgEtThr++){
        ...
        bool isFired = false;
        for(int i=0; i<EgSel_index.size(); i++){
          for(int j=0; j<EgSel_index.size(); j++){
            if(j >= i) continue;
            float dR = deltaR(egEta[EgSel_index[i]],egPhi[EgSel_index[i]],egEta[EgSel_index[j]],egPhi[EgSel_index[j]]);
            if(dR < 1.0 && egEt[EgSel_index[i]] >= EgEtThr && egEt[EgSel_index[j]] >= EgEtThr)
