...
void Frequency_plot_mu_MC(const TString samplename="KeeMC",
									const int nEvents = -1){
	...
	for(int MuEtThr = 2; MuEtThr < 11; MuEtThr+=1){
		...
		for(double MuEtaThr = 1.5; MuEtaThr < 1.6 ; MuEtaThr+=0.1){	
			...
			int counter_2 = 0; //count how many events fired trigger
			...
			while (std::getline(insample, line)){
				...
				cout<<Evt2Process_L1<<" events out of "<<eventTree_L1->GetEntries()<<" will be processed"<<endl;
