...
void Frequency_plot_9MC(const TString samplename="KeeMC",
											const int nEvents = -1,
	...
	// Histograms
	TH1F* hist_11 = new TH1F("iso","EG isolation flag (leading)", 4,0,4);
	TH1F* hist_12 = new TH1F("iso","EG isolation flag (sub-leading)", 4,0,4);
	...
	while (std::getline(insample, line)){
		...
		for(UInt_t ientry=0; ientry<Evt2Process_L1; ientry++){
			...
			hist_11->Fill(egIso[EgSel_index[0]]);
			hist_12->Fill(egIso[EgSel_index[1]]);
	...
	xaxis->SetTitle("EG isolation flag");
	xaxis->SetTitleOffset(1.2);
	hist_11->Draw("HIST");
	hist_12->SetLineColor(6);
	hist_12->Draw("HIST SAME");
	TString outName = "Histo_EGiso.png";
	c11->Print(outName);
