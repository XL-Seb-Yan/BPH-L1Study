...
fileList = rootDir + "ZeroBiasPrompt2018D.list" 
...
while ret == 0:
   ...
   for ichunk in range(chunks):
      ...
      with open(jobDir + jobName + str(count) + '.jdl', 'w') as jdl:
         ...
         jdl.write("Transfer_Input_Files = " + jobScript + ", " + jobCfg + ", " + filelist_name + ", JSON_dataNtuples.txt" + "\n")
         jdl.write("Output = "    + jobDir + "out/$(ProcId).out\n")
         jdl.write("Error = "     + jobDir + "err/$(ProcId).err\n")
         jdl.write("Log = "       + jobDir + "log/$(ProcId).log\n")
         jdl.write("Arguments = " + eosDir + " " + jobName + " " + rel + " " + str(ichunk) + " " + jobCfg + " " + filelist_name + " " + fileName + " " + proxy_path + "\n")
